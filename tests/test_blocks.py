...
async def test_list_segments(respx_mock):
    respx_mock.get("/v1/datasets/d_1/documents/doc_1/segments").mock(
...
async def test_add_segment(respx_mock):
    respx_mock.post("/v1/datasets/d_1/documents/doc_1/segments").mock(
        return_value=Response(200, json={"segment_id": "seg_1"})
    )
    async with DifyAsyncClient(base_url="http://localhost", api_key="test") as client:
