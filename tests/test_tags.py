...
async def test_list_kb_type_tags(respx_mock):
    respx_mock.get("/v1/metadata/kb-type-tags").mock(
...
async def test_bind_unbind_dataset(respx_mock):
    respx_mock.post("/v1/metadata/kb-type-tags/t_1/datasets/d_1").mock(
        return_value=Response(200, json={"ok": True})
    )
    respx_mock.delete("/v1/metadata/kb-type-tags/t_1/datasets/d_1").mock(
        return_value=Response(200, json={"ok": True})
    )
    async with DifyAsyncClient(base_url="http://localhost", api_key="test") as client:
