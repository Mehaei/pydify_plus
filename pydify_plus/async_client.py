        ...
        *,
        ...
        for attempt in range(_retries + 1):
            try:
                ...
                self.logger.debug(
                    f"{request_id}: Making {method} request to {url} (attempt {attempt + 1}/{_retries + 1})"
                )
                self.logger.debug(f"{request_id}: Request headers: {headers}")
                self.logger.debug(
                    f"{request_id}: Request JSON: {json}"
                    if json
                    else f"{request_id}: Request DATA: {data}"
                )
                ...
                last_exc = DifyTimeoutError(
                    f"Request timed out after {_timeout} seconds"
                )
                self.logger.warning(
                    f"Request timeout (attempt {attempt + 1}/{_retries + 1})"
                )

            except httpx.ConnectError as e:
                last_exc = DifyConnectionError(f"Connection error: {e}")
                self.logger.warning(
                    f"Connection error (attempt {attempt + 1}/{_retries + 1}): {e}"
                )

            except httpx.HTTPStatusError as e:
                request_id = (
                    e.response.headers.get("x-request-id") if e.response else None
                )
                ...
                if status_code == 401:
                    raise DifyAuthError(
                        status_code=status_code, body=data, request_id=request_id
                    ) from e
                elif status_code == 404:
                    raise DifyNotFoundError(
                        status_code=status_code, body=data, request_id=request_id
                    ) from e
    ...
    async def _stream_request(
        ...
        *,
        ...
        for attempt in range(_retries + 1):
            try:
                self.logger.debug(
                    f"Making streaming {method} request to {url} (attempt {attempt + 1}/{_retries + 1})"
                )
                ...
                async with aconnect_sse(
                    ...
                    self.logger.debug(
                        f"Streaming connection established successfully (attempt {attempt + 1})"
                    )
                    ...
                    try:
                        ...
                        request_id = (
                            response.headers.get("x-request-id") if response else None
                        )
                    ...
                    self.logger.debug(
                        f"Streaming request completed successfully (attempt {attempt + 1})"
                    )
                    return

            except httpx.TimeoutException:
                last_exc = DifyTimeoutError(
                    f"Streaming request timed out after {_timeout} seconds"
                )
                self.logger.warning(
                    f"Streaming request timeout (attempt {attempt + 1}/{_retries + 1})"
                )
                ...
                self.logger.warning(
                    f"Unexpected streaming error (attempt {attempt + 1}/{_retries + 1}): {e}"
                )
            ...
            if last_exc and attempt < _retries:
                delay = self.retry_backoff_factor * (2**attempt)
                self.logger.info(
                    f"Retrying streaming request in {delay:.2f} seconds..."
                )
                await asyncio.sleep(delay)
