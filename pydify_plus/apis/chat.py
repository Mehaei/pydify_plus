...
class ChatApi(BaseApi):
    ...
    async def create_chat_message(
        ...
        return await self.request(
            "POST", API_ENDPOINTS["CHAT_MESSAGES_CREATE"], json=payload
        )
    ...
    async def get_chat_message(self, message_id: str) -> dict:
        ...
        """
        return await self.request(
            "GET", API_ENDPOINTS["CHAT_MESSAGES_GET"].format(conversation_id=message_id)
        )
    ...
    async def upload_file_bytes(
        ...
        *,
        ...
        payload = {"user": user, "purpose": purpose}
        return await self.request(
            "POST", API_ENDPOINTS["FILES_UPLOAD"], files=files, json=payload
        )

    async def preview_file(
        self, file_id: str, as_attachment: bool = False
    ) -> dict[str, any]:
        """
        获取文件预览信息。
    ...
    async def stop_chat_message(self, task_id: str, user: str = "abc-123") -> dict:
        ...
        return await self.request(
            "POST",
            API_ENDPOINTS["CHAT_MESSAGES_STOP"].format(task_id=task_id),
            json=payload,
        )
        ...
        **kwargs,
        ...
        if httpx_client is None:
            httpx_client = httpx.AsyncClient(
                base_url=self._client.base_url, timeout=self._client.timeout
            )
            close_client = True
    ...
    def chat_message(
        ...
        for event in self.stream_chat_message_sync(
