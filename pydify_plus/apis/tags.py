...
class TagsApi:
    ...
    async def create_kb_type_tag(self, name: str) -> Dict[str, Any]:
        ...
        return await self.client._arequest(
            "POST", API_ENDPOINTS["KB_TYPE_TAGS_CREATE"], json={"name": name}
        )

    async def delete_kb_type_tag(self, tag_id: str) -> Dict[str, Any]:
        """删除知识库类型标签。"""
        return await self.client._arequest(
            "DELETE", API_ENDPOINTS["KB_TYPE_TAGS_DELETE"].format(tag_id=tag_id)
        )
    ...
    async def rename_kb_type_tag(self, tag_id: str, new_name: str) -> Dict[str, Any]:
        ...
        return await self.client._arequest(
            "POST",
            API_ENDPOINTS["KB_TYPE_TAGS_RENAME"].format(tag_id=tag_id),
            json={"name": new_name},
        )
    ...
    async def bind_dataset(self, tag_id: str, dataset_id: str) -> Dict[str, Any]:
        ...
        return await self.client._arequest(
            "POST",
            API_ENDPOINTS["KB_TYPE_TAGS_BIND_DATASET"].format(
                tag_id=tag_id, dataset_id=dataset_id
            ),
        )
    ...
    async def unbind_dataset(self, tag_id: str, dataset_id: str) -> Dict[str, Any]:
        ...
        return await self.client._arequest(
            "DELETE",
            API_ENDPOINTS["KB_TYPE_TAGS_UNBIND_DATASET"].format(
                tag_id=tag_id, dataset_id=dataset_id
            ),
        )
    ...
    async def list_dataset_bound_tags(self, dataset_id: str) -> Dict[str, Any]:
        """查询绑定到数据集的标签。"""
        return await self.client._arequest(
            "GET", API_ENDPOINTS["DATASET_BOUND_TAGS"].format(dataset_id=dataset_id)
        )
