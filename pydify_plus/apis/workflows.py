...
class WorkflowsApi(BaseApi):
    ...
    async def execute(
        self, workflow_id: str, *, inputs: Dict[str, Any], user: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        执行 workflow。
        """
        ...
        return await self.request(
            "POST",
            API_ENDPOINTS["WORKFLOW_EXECUTE"].format(workflow_id=workflow_id),
            json=payload,
        )
    ...
    async def execution_status(
        ...
        return await self.request(
            "GET",
            API_ENDPOINTS["WORKFLOW_EXECUTION_STATUS"].format(
    ...
    async def stop_task(self, workflow_id: str, execution_id: str) -> Dict[str, Any]:
        ...
        return await self.request(
            "POST",
            API_ENDPOINTS["WORKFLOW_STOP_TASK"].format(
                workflow_id=workflow_id, execution_id=execution_id
            ),
        )
    ...
    async def logs(self, workflow_id: str, execution_id: str) -> Dict[str, Any]:
        ...
        return await self.request(
            "GET",
            API_ENDPOINTS["WORKFLOW_LOGS"].format(
                workflow_id=workflow_id, execution_id=execution_id
            ),
        )
    ...
    async def upload_file(self, workflow_id: str, file_path: str) -> Dict[str, Any]:
        ...
        return await self.request(
            "POST",
            API_ENDPOINTS["WORKFLOW_FILES_UPLOAD"].format(workflow_id=workflow_id),
            files=files,
        )
